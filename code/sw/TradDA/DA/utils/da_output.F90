        ...
        do i = 1, da_2d_num+da_3d_num*num_lev
            do j = 1, latide-latids+1
                do k = 1, lonide-lonids+1
                     if (isnan(axb(i,j,k))) call log_error('bg nan')
                     if (isnan(axa(i,j,k))) call log_error('an nan')
                     !call mpi_abort()
                enddo
            enddo
        enddo
...
!!!!!!!!!!!!!
        ...
        !write(*,*) hash_key_amb, var_key, hash_key_axb, var_key
        ...
        do i=1, da_3d_num
            ...
            do ii = 1,lonide_out-lonids_out+1
                do jj = 1,latide_out-latids_out+1
                    k=1
                    do kk = mpas_num_lev_start, mpas_num_lev, zstep
                        tmp_3d(ii,jj,k) = spread_b(da_2d_num + (i - 1)*mpas_num_lev + kk, jj, ii)
                        k = k + 1
            ...
            do ii = 1,lonide_out-lonids_out+1
                do jj = 1,latide_out-latids_out+1
                    k=1
                    do kk = mpas_num_lev_start, mpas_num_lev, zstep
                        tmp_3d(ii,jj,k) = spread_a(da_2d_num + (i - 1)*mpas_num_lev + kk, jj, ii)
                        k = k + 1
