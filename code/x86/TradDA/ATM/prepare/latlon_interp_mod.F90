...
contains
  ...
  subroutine latlon_interp_bilinear_cell(src_lon, src_lat, src_data, dst_mesh, dst_data, extrap, zero_pole, ierr)
    ...
    allocate(data_sum(member_num,dst_mesh%full_ids:dst_mesh%full_ide))
    ...
    if (dst_mesh%has_south_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%full_ids:dst_mesh%full_ide,dst_mesh%full_jds)
        call zonal_sum_member(proc%zonal_circle, data_sum(:,:), pole(:))
        dst_data(:,dst_mesh%full_jds) = pole(1) / global_mesh%full_nlon
      end if
    end if
    if (dst_mesh%has_north_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%full_ids:dst_mesh%full_ide,dst_mesh%full_jde)
        call zonal_sum_member(proc%zonal_circle, data_sum, pole)
        dst_data(:,dst_mesh%full_jde) = pole(1) / global_mesh%full_nlon
      end if
    end if
  ...
  subroutine latlon_interp_bilinear_lon_edge(src_lon, src_lat, src_data, dst_mesh, dst_data, extrap, zero_pole, ierr)
    ...
    allocate(data_sum(member_num,dst_mesh%half_ids:dst_mesh%half_ide))
    ...
    if (dst_mesh%has_south_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%half_ids:dst_mesh%half_ide,dst_mesh%full_jds)
        call zonal_sum_member(proc%zonal_circle, data_sum , pole)
        dst_data(:,dst_mesh%full_jds) = pole(1) / global_mesh%half_nlon
      end if
    end if
    if (dst_mesh%has_north_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%half_ids:dst_mesh%half_ide,dst_mesh%full_jde)
        call zonal_sum_member(proc%zonal_circle, data_sum, pole)
        dst_data(:,dst_mesh%full_jde) = pole(1) / global_mesh%half_nlon
      end if
    end if
