...
!     case is needed at this level.
    IF( APPLYLEFT ) THEN
       IF( LASTV.GT.0 ) THEN
          ...
          CALL MPAL_GER( LASTV, LASTC, MPAL_VAL(-TAU), V, INCV, WORK, 1, C, LDC )
       END IF
    ELSE
       IF( LASTV.GT.0 ) THEN
          ...
          CALL MPAL_GER( LASTC, LASTV, MPAL_VAL(-TAU), WORK, 1, V, INCV, C, LDC )
       END IF
    END IF
    RETURN
...
! *
                    CALL MPAL_GEMV( 'Transpose', J-I, I-1, MPAL_VAL(-TAU( I )), &
...
! *
                    CALL MPAL_GEMV( 'No transpose', I-1, J-I, MPAL_VAL(-TAU( I )), &
...
! *
                        CALL MPAL_GEMV( 'Transpose', N-K+I-J, K-I, MPAL_VAL(-TAU( I )), &
...
! *
                        CALL MPAL_GEMV( 'No transpose', K-I, N-K+I-J, &
                             MPAL_VAL(-TAU( I )), V( I+1, J ), LDV, V( I, J ), LDV, &
                             ONE, T( I+1, I ), 1 )
                    END IF
! *
