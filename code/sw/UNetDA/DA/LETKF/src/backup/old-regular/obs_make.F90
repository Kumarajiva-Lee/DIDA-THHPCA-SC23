...
subroutine obs_interpolation_3d_2(xb, axb, ph, obs_d, obs_i, t1, t2)
    ...
    type(obs_struct), intent(inout) :: obs_d
    ...
    integer :: ph_pos(ens, obs_d%obs_l)

    do k = 1, ens
        j = 1
        do i = 1, obs_d%obs_l
            do while (j <= nz .and. obs_d%ph(i, obs_i) - ph(k, j) > 0.00001d0)
                j = j + 1
            end do
            ph_pos(k, i) = j - 1
#if (DEBUG == 1)
    ...
    tmp = 0
    do k = 1, obs_d%obs_v
        if (da_var_lev(obs_d%obs_vid(k)) > 1) then
            do l = 1, obs_d%obs_l
                tmp = tmp + 1
                do i = 1, ens
                    if (ph_pos(i, l) >= 1 .and. ph_pos(i, l) < nz) then
                        tl = da_var_start(obs_d%obs_vid(k)) + ph_pos(i, l) - 1
                        ...
                        r1 = r1 * t2 + r2 * (1 - t2)
                        r3 = r3 * t2 + r4 * (1 - t2)
                        obs_d%pro_yb(i, tmp, obs_i) = r1 * t3 + r3 * (1 - t3)
                    else
                        if (ph_pos(i, l) == 0) then
                            tl = da_var_start(obs_d%obs_vid(k))
                        else
                            tl = da_var_start(obs_d%obs_vid(k)) + nz - 1
                        end if
                        r1 = (xb(i, tl, 1, 1) + axb(tl, 1, 1)) * t1 + &
                        (xb(i, tl, 2, 1) + axb(tl, 2, 1)) * (1 - t1)
                        r2 = (xb(i, tl, 1, 2) + axb(tl, 1, 2)) * t1 + &
                        (xb(i, tl, 2, 2) + axb(tl, 2, 2)) * (1 - t1)
                        obs_d%pro_yb(i, tmp, obs_i) = r1 * t2 + r2 * (1 - t2)
                    end if
                end do
            end do
        else
            tl = obs_var_start(obs_d%obs_vid(k))
            tmp = tmp + 1
            do i = 1, ens
                r1 = (xb(i, tl, 1, 1) + axb(tl, 1, 1)) * t1 + &
                (xb(i, tl, 2, 1) + axb(tl, 2, 1)) * (1 - t1)
...
subroutine ps2ph(ps, aps, ph, t1, t2)
