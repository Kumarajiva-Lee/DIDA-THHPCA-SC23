...
contains
  ...
  subroutine atm_run(comm , group_id)
    ...
    ! if (is_root_proc()) then
    !   write(*,*) global_mesh%de_lon(1),global_mesh%de_lon(2),global_mesh%de_lon(179),global_mesh%de_lon(180)
    !   write(*,*) global_mesh%de_lat(1),global_mesh%de_lat(2),global_mesh%de_lat(179),global_mesh%de_lat(180)
    !   write(*,*) global_mesh%le_lon(1),global_mesh%le_lon(2),global_mesh%le_lon(179),global_mesh%le_lon(180)
    !   write(*,*) global_mesh%le_lat(1),global_mesh%le_lat(2),global_mesh%le_lat(179),global_mesh%le_lat(180)
    !   write(*,*) global_mesh%full_tangent_wgt(1,1),global_mesh%full_tangent_wgt(1,2),global_mesh%full_tangent_wgt(1,179),global_mesh%full_tangent_wgt(1,180)
    !   write(*,*) global_mesh%full_tangent_wgt(2,1),global_mesh%full_tangent_wgt(2,2),global_mesh%full_tangent_wgt(2,179),global_mesh%full_tangent_wgt(2,180)
    !   write(*,*) global_mesh%half_tangent_wgt(1,1),global_mesh%half_tangent_wgt(1,2),global_mesh%half_tangent_wgt(1,179),global_mesh%half_tangent_wgt(1,180)
    !   write(*,*) global_mesh%half_tangent_wgt(2,1),global_mesh%half_tangent_wgt(2,2),global_mesh%half_tangent_wgt(2,179),global_mesh%half_tangent_wgt(2,180)
    !   write(*,*) global_mesh%full_f(1),global_mesh%full_f(2),global_mesh%full_f(179),global_mesh%full_f(180)
    !   write(*,*) global_mesh%half_f(1),global_mesh%half_f(2),global_mesh%half_f(179),global_mesh%half_f(180)
    ! end if

    ! if (is_root_proc()) then
    !   write(*,*) global_mesh%full_lev(:)
    !   write(*,*) global_mesh%half_lev(:)
    ! end if

    if (is_root_proc()) then
      write(*,*) blocks(1)%static%gzs(1,1,1),blocks(1)%static%gzs(1,2,1),blocks(1)%static%gzs(1,359,1),blocks(1)%static%gzs(1,360,1)
      write(*,*) blocks(1)%dstate(old)%u_lon(1,1,1,1),blocks(1)%dstate(old)%u_lon(1,2,1,1),blocks(1)%dstate(old)%u_lon(1,359,1,1),blocks(1)%dstate(old)%u_lon(1,360,1,1)
      write(*,*) blocks(1)%dstate(old)%u_lon(1,1,2,1),blocks(1)%dstate(old)%u_lon(1,2,2,1),blocks(1)%dstate(old)%u_lon(1,359,2,1),blocks(1)%dstate(old)%u_lon(1,360,2,1)
    end if
    ...
    do while (.not. time_is_finished())
      ...
      !nc_flag = time_is_alerted('history_write')
        ...
        if (time_is_alerted('with_letkf')) then
          if (present(group_id)) then
            call output(new,group_id, flag = nc_flag)
          else
            call output(new, flag = nc_flag)
          endif
          da_cycle = da_cycle + 1
          if (da_cycle_total == -1 .or. da_cycle <= da_cycle_total) then
            if (da_cycle == 2) then
              call MPI_Barrier(comm, ierr)
        ...
        ! if (present(group_id)) then
        !   call output(old,group_id = group_id,flag = nc_flag)
        ! else
        !   call output(old, flag = nc_flag)
        ! endif

        if (present(group_id)) then
          call output(old,group_id = group_id)
        else
          call output(old)
        endif
  ...
  subroutine output(itime , group_id , new_file_in, flag)
    ...
    if (flag_in) then
      outputflag = .true.
    else if (time_is_alerted('history_write')) then
      outputflag = .true.
    else
      outputflag = .false.
    end if
    ...
    do iblk = 1, size(blocks)
      ...
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
          do i = mesh%half_ids, mesh%half_ide
            te(:) = te(:) + dstate%mfx_lon(:,i,j,k) * 0.5_r8 * dstate%u_lon(:,i,j,k) * mesh%area_lon(j) * 2
          end do
        end do
        do j = mesh%half_jds, mesh%half_jde
          do i = mesh%full_ids, mesh%full_ide
            te(:) = te(:) + dstate%mfy_lat(:,i,j,k) * 0.5_r8 * dstate%v_lat(:,i,j,k) * mesh%area_lat(j) * 2
          end do
        end do
      end do
      ...
      if (hydrostatic) then
        ...
        do k = mesh%full_kds, mesh%full_kde
          do j = mesh%full_jds, mesh%full_jde
            do i = mesh%full_ids, mesh%full_ide
              te(:) = te(:) + dstate%m(:,i,j,k) * cpd * dstate%t(:,i,j,k) * mesh%area_cell(j)
            end do
          end do
        end do
        do j = mesh%full_jds, mesh%full_jde
          do i = mesh%full_ids, mesh%full_ide
            te(:) = te(:) + static%gzs(:,i,j) * dstate%phs(:,i,j) * mesh%area_cell(j)
          end do
        end do
        ...
        do j = mesh%full_jds, mesh%full_jde
          do i = mesh%full_ids, mesh%full_ide
            te(:) = te(:) + (dstate%m(:,i,j,1)**2 * g * 0.5_r8 + dstate%m(:,i,j,1) * static%gzs(:,i,j)) * mesh%area_cell(j)
          end do
        end do
      end if
