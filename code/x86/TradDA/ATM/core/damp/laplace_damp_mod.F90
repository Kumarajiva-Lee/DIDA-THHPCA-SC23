module laplace_damp_mod
  ...
  contains
    ...
    subroutine laplace_damp_on_cell_2d(block, order, f, coef, lat_coef, fill)
      ...
      real(r8) gx1(member_num , & 
                   block%mesh%full_ims:block%mesh%full_ime, &
                   block%mesh%full_jms:block%mesh%full_jme)
      real(r8) gx2(member_num , & 
                   block%mesh%full_ims:block%mesh%full_ime, &
                   block%mesh%full_jms:block%mesh%full_jme)
      real(r8) gy1(member_num , & 
                   block%mesh%full_ims:block%mesh%full_ime, &
                   block%mesh%full_jms:block%mesh%full_jme)
      real(r8) gy2(member_num , & 
                   block%mesh%full_ims:block%mesh%full_ime, &
                   block%mesh%full_jms:block%mesh%full_jme)
      ...
      do k = 1, (order - 2) / 2
        ...
        call fill_halo_member(block, gx2, full_lon=.true., full_lat=.true., south_halo=.false., north_halo=.false., async=block%dstate(1)%async(async_tmp))
        call block%dstate(1)%async(async_tmp)%wait()
        call fill_halo_member(block, gy2, full_lon=.true., full_lat=.true., west_halo=.false., east_halo=.false., async=block%dstate(1)%async(async_tmp))
        call block%dstate(1)%async(async_tmp)%wait()
        gx1 = gx2
        gy1 = gy2
      end do
      ...
      if (merge(fill, .true., present(fill))) then
        call fill_halo_member(block, f, full_lon=.true., full_lat=.true., async=block%dstate(1)%async(async_tmp))
        call block%dstate(1)%async(async_tmp)%wait()
      end if
    ...
    subroutine laplace_damp_on_cell_3d(block, order, f, coef, lat_coef, lev_coef)
      ...
      integer k
        ...
        !!!!!!!!!No State!!!!!!!!!
        call fill_halo_member(block, f, full_lon=.true., full_lat=.true., full_lev=.true.,async=block%dstate(1)%async(async_tmp))
        call block%dstate(1)%async(async_tmp)%wait()
    ...
    subroutine laplace_damp_on_lon_edge(block, order, f, coef, lat_coef, lev_coef)
        ...
        do k = mesh%full_kds, mesh%full_kde
          ...
          do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
            do i = mesh%full_ids, mesh%full_ide + 1
              do im = 1 , member_num
                fx(im,i,j) = s * sum(f(im,i-ns:i+ns+1,j,k) * w(:2*ns))
              end do
            end do
          end do
          do j = mesh%half_jds - 1, mesh%half_jde
            do i = mesh%half_ids, mesh%half_ide
              do im = 1 , member_num
                fy(im,i,j) = s * sum(f(im,i,j-ns+1:j+ns,k) * w(:2*ns))
              end do
            end do
          end do
          ! Limit damping flux to avoid upgradient (Xue 2000).
      ...
      !!!!!!!!!No State!!!!!!!!!
        call fill_halo_member(block, f, full_lon=.false., full_lat=.true., full_lev=.true.,async=block%dstate(1)%async(async_tmp))
        call block%dstate(1)%async(async_tmp)%wait()
      end associate
      ...
      associate (mesh => block%mesh)
        ...
        do k = mesh%full_kds, mesh%full_kde
          do j = mesh%half_jds, mesh%half_jde
            do i = mesh%half_ids - 1, mesh%half_ide
              do im = 1 , member_num
                fx(im,i,j) = s * sum(f(im,i-ns+1:i+ns,j,k) * w(:2*ns))
              end do
            end do
          end do
          do j = mesh%full_jds, mesh%full_jde_no_pole + 1
            do i = mesh%full_ids, mesh%full_ide
              do im = 1 , member_num
                fy(im,i,j) = s * sum(f(im,i,j-ns:j+ns+1,k) * w(:2*ns))
              end do
            end do
          end do
          ! Limit damping flux to avoid upgradient (Xue 2000).
        ...
        !!!!!!!!!No State!!!!!!!!!
        call fill_halo_member(block, f, full_lon=.true., full_lat=.false., full_lev=.true., async=block%dstate(1)%async(async_tmp))
        call block%dstate(1)%async(async_tmp)%wait()
      end associate
