...
#ifdef RPE_ON
    add%rpe_st%sbits = MAX(significand_bits(a%rpe_st), significand_bits(b%rpe_st))
...
#ifdef RPE_ON
    sub%rpe_st%sbits = MAX(significand_bits(a%rpe_st), significand_bits(b%rpe_st))
...
#ifdef RPE_ON
    multi%rpe_st%sbits = MAX(significand_bits(a%rpe_st), significand_bits(b%rpe_st))
...
#ifdef RPE_ON
    div%rpe_st%sbits = MAX(significand_bits(a%rpe_st), significand_bits(b%rpe_st))
...
type(mpal_st) function add_const(a, b)
    ...
    add_const%svalue = a%svalue + real(b, 4)
...
#ifdef RPE_ON
    add_const%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
type(mpal_st) function sub_const(a, b)
    ...
    sub_const%svalue = a%svalue - real(b, 4)
...
#ifdef RPE_ON
    sub_const%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
type(mpal_st) function multi_const(a, b)
    ...
    multi_const%svalue = a%svalue * real(b, 4)
...
#ifdef RPE_ON
    multi_const%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
type(mpal_st) function div_const(a, b)
    ...
    div_const%svalue = a%svalue / real(b, 4)
...
#ifdef RPE_ON
    div_const%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
type(mpal_st) function const_add(a, b)
    ...
    const_add%svalue = real(a, 4)+ b%svalue
...
#ifdef RPE_ON
    const_add%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
type(mpal_st) function const_sub(a, b)
    ...
    const_sub%svalue = real(a, 4) - b%svalue
...
#ifdef RPE_ON
    const_sub%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
type(mpal_st) function const_multi(a, b)
    ...
    const_multi%svalue = real(a, 4) * b%svalue
...
#ifdef RPE_ON
    const_multi%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
type(mpal_st) function const_div(a, b)
    ...
    const_div%svalue = real(a, 4) / b%svalue
...
#ifdef RPE_ON
    const_div%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    add_const_s%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    sub_const_s%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    multi_const_s%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    div_const_s%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    add_const_i%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    sub_const_i%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    multi_const_i%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    div_const_i%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
...
#ifdef RPE_ON
    power_const_i%rpe_st%sbits = max(significand_bits(a), significand_bits(b))
