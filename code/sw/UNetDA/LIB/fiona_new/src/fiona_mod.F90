...
contains
  ...
  subroutine fiona_add_var(dataset_name, name, long_name, units, dim_names, data_type, deflate_level, missing_value)
  ...
  subroutine fiona_start_output(dataset_name, time_in_seconds, new_file, tag)
    ...
    if (dataset%proc_id == dataset%group_id) then
      call log_notice('Start to write on group ' // to_str(dataset%group_id) // '.')
    end if
  ...
  subroutine apply_dataset_to_netcdf_master(file_path, dataset, new_file)
    ...
    do while (.not. iter%ended())
      ...
      if (ierr /= NF90_NOERR) then
        ...
        if (var%deflate_level /= 0) then
          ierr = NF90_DEF_VAR_DEFLATE(dataset%id, var%id, shuffle=1, deflate=1, deflate_level=var%deflate_level)
          call handle_error(ierr, 'Failed to define variable deflate level!', __FILE__, __LINE__)
        end if
        deallocate(dimids)
  ...
  subroutine fiona_end_output(dataset_name)
    ...
    if (dataset%proc_id == dataset%group_id) then
      call log_notice('End to write on group ' // to_str(dataset%group_id) // '.')
    end if
