...
SUBROUTINE MPAL_DLARFG( N, ALPHA, X, INCX, TAU )
    ...
    INTEGER            J, KNT
    TYPE(MPAL_ST)      BETA
    DOUBLE PRECISION   RSAFMN, SAFMIN, XNORM
    ...
    IF( XNORM.EQ.ZERO ) THEN
        TAU = ZERO
    ELSE
        BETA = -SIGN( DLAPY2( MPAL_VAL(ALPHA), XNORM ), MPAL_VAL(ALPHA) )
        SAFMIN = DLAMCH( 'S' ) / DLAMCH( 'E' )
        KNT = 0
        IF( ABS( MPAL_VAL(BETA) ).LT.SAFMIN ) THEN
            RSAFMN = ONE / SAFMIN
            10  CONTINUE
            KNT = KNT + 1
            ...
            BETA = BETA*RSAFMN
            ALPHA = ALPHA*RSAFMN
            IF( (ABS( MPAL_VAL(BETA) ).LT.SAFMIN) .AND. (KNT .LT. 20) ) GO TO 10

            XNORM = MPAL_DNRM2( N-1, X, INCX )
            BETA = -SIGN( DLAPY2( MPAL_VAL(ALPHA), XNORM ), MPAL_VAL(ALPHA) )
        END IF
        TAU = ( BETA-ALPHA ) / BETA
        CALL MPAL_DSCAL( N-1, ONE / ( MPAL_VAL(ALPHA)-MPAL_VAL(BETA) ), X, INCX )
            ...
            IF( I.LT.N ) THEN
                ...
                IF( I.LT.N ) THEN
                    ...
                    CALL MPAL_GEMV( 'No transpose', I-1, N-I, -ONE, &
                        ...
                        W( 1, IW ), 1 )
                END IF
                CALL MPAL_DSCAL( I-1, MPAL_VAL(TAU( I-1 )), W( 1, IW ), 1 )
                ALPHA = -HALF*MPAL_VAL(TAU( I-1 ))*MPAL_DOT( I-1, W( 1, IW ), 1, &
                    A( 1, I ), 1 )
        ...
        ELSE
            DO 20 I = 1, NB
                ...
                IF( I.LT.N ) THEN
                   ...
                   CALL MPAL_DSCAL( N-I, MPAL_VAL(TAU( I )), W( I+1, I ), 1 )
                   ALPHA = -HALF*MPAL_VAL(TAU( I ))*MPAL_DOT( N-I, W( I+1, I ), 1, &
                        A( I+1, I ), 1 )
...
SUBROUTINE MPAL_DSYTD2( UPLO, N, A, LDA, D, E, TAU, INFO )
        ...
        IF( N.LE.0 ) RETURN
            ...
            DO 10 I = N - 1, 1, -1
                ...
                E( I ) = A( I, I+1 )
                IF( MPAL_VAL(TAUI).NE.ZERO ) THEN
                    
                   A( I, I+1 ) = ONE
                   CALL MPAL_SYMV( UPLO, I, MPAL_VAL(TAUI), A, LDA, A( 1, I+1 ), 1, ZERO, &
                           TAU, 1 )

                   ALPHA = -HALF*MPAL_VAL(TAUI)*MPAL_DOT( I, TAU, 1, A( 1, I+1 ), 1 )
...
! *
                    CALL MPAL_SYMV( UPLO, N-I, MPAL_VAL(TAUI), A( I+1, I+1 ), LDA, &
...
! *
                    ALPHA = -HALF*MPAL_VAL(TAUI)*MPAL_DOT( N-I, TAU( I ), 1, A( I+1, I ), &
                                1 )
