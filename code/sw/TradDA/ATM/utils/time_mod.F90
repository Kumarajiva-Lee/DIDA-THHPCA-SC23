module time_mod
  ...
  public curr_time
  public start_time_type
  public end_time_type
  ...
  type(datetime_type) start_time_type
  type(datetime_type) end_time_type
...
contains
  ...
  subroutine time_init(dt_in_seconds)
    ...
    if (sum(start_time_array) > 0) then
      start_time_type = create_datetime(year=start_time_array(1),  &
                                   month=start_time_array(2), &
                                   day=start_time_array(3),   &
                                   hour=start_time_array(4),  &
                                   minute=start_time_array(5))
    else
      start_time_type = create_datetime(year=1, month=1, day=1, hour=0, minute=0)
    end if
    if (run_days > 0 .or. run_hours > 0) then
      end_time_type = start_time_type + create_timedelta(days=run_days, hours=run_hours)
    else if (sum(end_time_array) > 0) then
      end_time_type = create_datetime(year=end_time_array(1),  &
    ...
    curr_time = start_time_type

    start_time_str = start_time_type%format('%Y-%m-%dT%H_%M_%S')
  ...
  subroutine time_fast_forward(time_value, time_units)
    ...
    skipped_time = curr_time - start_time_type
  ...
  subroutine time_add_alert(name, months, days, hours, minutes, seconds, first_alert )
    ...
    if (first_alert) then
      alert%last_time = start_time_type - alert%period
    else
      alert%last_time = curr_time
    end if
