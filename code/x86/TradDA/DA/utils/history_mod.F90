...
contains
  ...
  subroutine history_init(member_num)
    ...
    integer im
    character*20 s_member , fid0 , fid1
    ...
    do im = 1 , member_num
      ...
      if (baroclinic) then
        ...
        call fiona_add_var(fid1, 'pt_lev'   , long_name='potential temperature on half levels'          , units='', dim_names=cell_lev_dims)
        call fiona_add_var(fid1, 'ph_lev'   , long_name='hydrostatic pressure on half levels'           , units='', dim_names=cell_lev_dims)
        ...
        call fiona_add_var(fid1, 'wedphdlev', long_name='vertical coordinate velocity'                  , units='', dim_names=lev_dims)
        call fiona_add_var(fid1, 'wedphdlev_lon', long_name='vertical coordinate velocity'              , units='', dim_names=lon_lev_dims)
        call fiona_add_var(fid1, 'wedphdlev_lat', long_name='vertical coordinate velocity'              , units='', dim_names=lat_lev_dims)
        ...
        call fiona_add_var(fid1, 'm_lon'    , long_name='dph on lon edges on full levels'               , units='', dim_names=lon_dims)
        call fiona_add_var(fid1, 'm_lat'    , long_name='dph on lat edges on full levels'               , units='', dim_names=lat_dims)
        call fiona_add_var(fid1, 'm_vtx'    , long_name='dph on vertices on full levels'                , units='', dim_names=vtx_dims)
        call fiona_add_var(fid1, 'pv_lon'   , long_name='pv on U grid'                                  , units='', dim_names=lon_dims)
        call fiona_add_var(fid1, 'pv_lat'   , long_name='pv on V grid'                                  , units='', dim_names=lat_dims)
        call fiona_add_var(fid1, 'ke'       , long_name='kinetic energy on cell grid'                   , units='', dim_names=cell_dims)
        call fiona_add_var(fid1, 'wp'       , long_name='omega'                                         , units='', dim_names=cell_dims)
        call fiona_add_var(fid1, 'div'      , long_name='divergence'                                    , units='', dim_names=cell_dims)
      else
        ...
        call fiona_add_var(fid1, 'm_lon'    , long_name='mass on lon edges'                             , units='', dim_names=lon_dims_2d)
        call fiona_add_var(fid1, 'm_lat'    , long_name='mass on lat edges'                             , units='', dim_names=lat_dims_2d)
        call fiona_add_var(fid1, 'm_vtx'    , long_name='mass on vertices'                              , units='', dim_names=vtx_dims_2d)
        call fiona_add_var(fid1, 'pv_lon'   , long_name='pv on U grid'                                  , units='', dim_names=lon_dims_2d)
        call fiona_add_var(fid1, 'pv_lat'   , long_name='pv on V grid'                                  , units='', dim_names=lat_dims_2d)
        call fiona_add_var(fid1, 'ke'       , long_name='kinetic energy on cell grid'                   , units='', dim_names=cell_dims_2d)
        call fiona_add_var(fid1, 'div'      , long_name='divergence'                                    , units='', dim_names=cell_dims_2d)
      end if
    end do
  ...
  subroutine history_write_state(blocks, itime , member_num) !need to add member
    ...
    integer iblk, is, ie, js, je, ks, ke , im
    integer start(3), count(3)
    character*20 fid0 , fid1 , s_member
    
    !need to fix member output
    do im = 1 , member_num
  ...
  subroutine history_write_debug(blocks, itime , member_num)
    ...
    do im = 1 , member_num
      ...
      call fiona_output(fid1, 'div'     , state%div     (im,is:ie,js:je,ks:ke), start=start, count=count)
      if (baroclinic) then
        ...
        call fiona_output(fid1, 'wp'      , state%wp     (im,is:ie,js:je,ks:ke), start=start, count=count)
      else
      ...
      if (baroclinic) then
        call fiona_output(fid1, 'wedphdlev', state%wedphdlev(im,is:ie,js:je,ks:ke), start=start, count=count)
        call fiona_output(fid1, 'pt_lev'   , state%pt_lev   (im,is:ie,js:je,ks:ke), start=start, count=count)
        call fiona_output(fid1, 'ph_lev'   , state%ph_lev   (im,is:ie,js:je,ks:ke), start=start, count=count)
      end if
      ...
      if (baroclinic) then
        call fiona_output(fid1, 'wedphdlev_lon', state%wedphdlev_lon(im,is:ie,js:je,ks:ke), start=start, count=count)
      end if
      ...
      if (baroclinic) then
        call fiona_output(fid1, 'wedphdlev_lat', state%wedphdlev_lat(im,is:ie,js:je,ks:ke), start=start, count=count)
      end if
