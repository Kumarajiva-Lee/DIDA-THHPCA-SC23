...
contains
  ...
  subroutine div_damp_run(block, dstate)
    ...
    call Add_Function("div_damp_run")
    call Indent_In()
#endif


      call dstate%async(async_div)%wait()
      call dstate%async(async_div2)%wait()
    ...
    call Get_Start_Time(tran_time_start)
      ...
      call fill_halo_member(block, u, full_lon=.true., full_lat=.true. , full_lev=.true., async=dstate%async(async_u_lon))
