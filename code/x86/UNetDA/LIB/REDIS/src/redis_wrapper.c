                        ...
                        float *lon_buf, float *lat_buf, float *lev_buf, float *val_buf, int *nn)
{
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnf(cc, reply, lon_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnf(cc, reply, lat_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnf(cc, reply, lev_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnf(cc, reply, val_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
                        ...
                        double *lon_buf, double *lat_buf, double *lev_buf, double *val_buf, int *nn)
{
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnd(cc, reply, lon_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnd(cc, reply, lat_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnd(cc, reply, lev_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
    ...
    mm = 0;
    reply = redisClusterCommand(cc, sb_concat(key));
    errno = reply_returnd(cc, reply, val_buf, &mm);
    if (errno != 0) return errno;
    freeReplyObject(reply);
    sb_reset(key);
