...
contains
  ...
  subroutine filter_init(this, mesh, type)
    ...
    if (proc.id == 0) then
      ! write(*,*) mesh%full_jds_no_pole,mesh%full_jde_no_pole,mesh%half_jds,mesh%half_jde
      ! write(*,*) "ngrid_lon"
      ! do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
      !   write(*,*) j , this%ngrid_lon(j)
      ! end do
      ! write(*,*) "ngrid_lat"
      ! do j = mesh%half_jds, mesh%half_jde
      !   write(*,*) j , this%ngrid_lat(j)
      ! end do
      !write(*,*) "lat0" , global_mesh%full_lat_deg(2),global_mesh%half_lat_deg(1)
    end if
    ...
    if (proc.id == 0) then
      ...
      !   write(*,*) j , this%ngrid_lat(j)
      ! end do
      write(*,*) this%ngrid_lon(mesh%full_jds_no_pole), this%ngrid_lat(mesh%half_jds)
      write(*,*) this%wgt_lon(1,mesh%full_jds_no_pole),this%wgt_lon(this%ngrid_lon(mesh%full_jds_no_pole),mesh%full_jds_no_pole),this%wgt_lat(1,mesh%half_jds),this%wgt_lat(this%ngrid_lat(mesh%half_jds),mesh%half_jds)
    end if
