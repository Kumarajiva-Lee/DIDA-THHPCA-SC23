module coupler_mod
  ...
  subroutine init_coupler()
    ...
    if ((atm_group + da_group + io_group) .lt. atm_stride) then
    ...
    cinfo%io_np = io_group * io_group_num
    ...
    if((cinfo%atm_np + cinfo%da_np + cinfo%io_np) .lt. cinfo%globalsize) then
        call log_warning_root("atm procs add da procs < total proc size! &
        maybe some node is free!")
    end if

    if((cinfo%atm_np + cinfo%da_np + cinfo%io_np) .gt. cinfo%globalsize) then
    ...
    do i = 0, io_group_num - 1
      do j = 0, io_group - 1
        color(io_stride * i + io_root + j) = IO_Color
        snode_color(io_stride * i + io_root + j) = i
        atm_gcolor(io_stride * i + io_root + j) = IO_Color
        snode_atmgcolor(io_stride * i + io_root + j) = IO_Color
      end do
    end do
