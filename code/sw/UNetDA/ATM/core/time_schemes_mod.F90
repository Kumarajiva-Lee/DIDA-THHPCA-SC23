    ...
    call Get_Start_Time(cal_time_start)
        ...
        do j = mesh%full_jds, mesh%full_jde
          do i = mesh%full_ids, mesh%full_ide
           ...
           ! if (i == 1 .and. j == 1) write(*,*) "phs",time_step,new_state%phs(1,i,j),old_state%phs(1,i,j),dtend%dphs(1,i,j)
          end do
        end do
    ...
    call Get_Start_Time(cal_time_start)
        ...
        do k = mesh%full_kds, mesh%full_kde
          do j = mesh%full_jds, mesh%full_jde
            ...
            !  if (i == 1 .and. j == 1 .and. k == 1) write(*,*) "pt" ,time_step,new_state%pt(1,i,j,k),old_state%pt(1,i,j,k),old_state%m(1,i,j,k),dtend%dpt(1,i,j,k),new_state%m(1,i,j,k) 
            end do
          end do
        end do
    ...
    call Get_Start_Time(cal_time_start)
        ...
        do k = mesh%full_kds, mesh%full_kde
          do j = mesh%full_jds, mesh%full_jde
            ...
            !  if (i == 1 .and. j == 1 .and. k == 1) write(*,*) "gz" ,time_step,new_state%gz(1,i,j,k),old_state%gz(1,i,j,k),dtend%dgz(1,i,j,k)
            end do
          end do
        end do
      ...
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
          do i = mesh%half_ids, mesh%half_ide
           ...
           ! if (i == 1 .and. j == 2 .and. k == 1) write(*,*) "u_lon" ,time_step, new_state%u_lon(1,i,j,k) ,old_state%u_lon(1,i,j,k) , dtend%du(1,i,j,k)
          end do
        end do
      end do
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%half_jds, mesh%half_jde
          ...
          !  if (i == 1 .and. j == 1 .and. k == 1) write(*,*) "v_lat" ,time_step, new_state%v_lat(1,i,j,k) , old_state%v_lat(1,i,j,k) , dtend%dv(1,i,j,k)
          end do
        end do
      end do
