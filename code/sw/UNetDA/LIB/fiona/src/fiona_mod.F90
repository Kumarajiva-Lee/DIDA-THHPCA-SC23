...
contains
  ...
  subroutine fiona_init(time_units, start_time)
    ...
    if (present(time_units)) then
      ...
      case default
        call log_error('Invalid time_units ' // trim(time_units) // '!', __FILE__, __LINE__)
      end select
    end if
  ...
  subroutine fiona_start_output(dataset_name, time_in_seconds, new_file, tag)
    ...
    !call log_notice("start_output: " // file_path)
...
#ifdef HAS_MPI
      !call log_notice("has_mpi")
      if (dataset%mpi_comm == MPI_COMM_NULL) then
        ierr = NF90_CREATE(file_path, NF90_NETCDF4, dataset%id)
      else
        !call log_notice("not null")
        ierr = NF90_CREATE(file_path, ior(NF90_NETCDF4, NF90_MPIIO), dataset%id, comm=dataset%mpi_comm, info=MPI_INFO_NULL)
      end if
      if (ierr == -114) then ! Parallel operation on file opened for non-parallel access
        !call log_notice("error -114")
        dataset%mpi_comm = MPI_COMM_NULL
        ierr = NF90_CREATE(file_path, NF90_NETCDF4, dataset%id)
      end if
#else
      !call log_notice("not mpi")
      ierr = NF90_CREATE(file_path, NF90_NETCDF4, dataset%id)
#endif
      !call log_notice(file_path)
      call handle_error(ierr, 'Failed to create NetCDF file to output!', __FILE__, __LINE__)
    end if
  ...
  subroutine fiona_close_dataset(dataset_name)
  
     character(*), intent(in) :: dataset_name

     call datasets%remove(trim(dataset_name))

  end subroutine fiona_close_dataset
