module allocator_mod
  ...
  interface allocate_array
    ...
    module procedure allocate_array_3d_member_i4
...
contains
  ...
  subroutine allocate_array_1d_member_r8(mesh, array, member , full_lon, half_lon, full_lat, half_lat)
    ...
    if (present(full_lon)) then
      if (full_lon) allocate(array(member , mesh%full_ims:mesh%full_ime))
    else if (present(half_lon)) then
      if (half_lon) allocate(array(member , mesh%half_ims:mesh%half_ime))
    else if (present(full_lat)) then
      if (full_lat) allocate(array(member , mesh%full_jms:mesh%full_jme))
    else if (present(half_lat)) then
      if (half_lat) allocate(array(member , mesh%half_jms:mesh%half_jme))
    else
  ...
  subroutine allocate_array_2d_member_r8(mesh, array , member , full_lon, half_lon, full_lat, half_lat)
    ...
    if (present(full_lon) .and. present(full_lat)) then
      if (full_lon .and. full_lat) allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%full_jms:mesh%full_jme))
    else if (present(full_lon) .and. present(half_lat)) then
      if (full_lon .and. half_lat) allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%half_jms:mesh%half_jme))
    else if (present(half_lon) .and. present(full_lat)) then
      if (half_lon .and. full_lat) allocate(array(member,mesh%half_ims:mesh%half_ime,mesh%full_jms:mesh%full_jme))
    else if (present(half_lon) .and. present(half_lat)) then
      if (half_lon .and. half_lat) allocate(array(member,mesh%half_ims:mesh%half_ime,mesh%half_jms:mesh%half_jme))
    else
      allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%full_jms:mesh%full_jme))
    end if
  ...
  subroutine allocate_array_3d_member_r8(mesh, array, member , full_lon, half_lon, full_lat, half_lat, full_lev, half_lev)
    ...
    if (present(full_lon) .and. present(full_lat) .and. present(full_lev)) then
      if (full_lon .and. full_lat .and. full_lev) allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%full_jms:mesh%full_jme,mesh%full_kms:mesh%full_kme))
    else if (present(full_lon) .and. present(half_lat) .and. present(full_lev)) then
      if (full_lon .and. half_lat .and. full_lev) allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%half_jms:mesh%half_jme,mesh%full_kms:mesh%full_kme))
    else if (present(full_lon) .and. present(full_lat) .and. present(half_lev)) then
      if (full_lon .and. full_lat .and. half_lev) allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%full_jms:mesh%full_jme,mesh%half_kms:mesh%half_kme))
    else if (present(full_lon) .and. present(half_lat) .and. present(half_lev)) then
      if (full_lon .and. half_lat .and. half_lev) allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%half_jms:mesh%half_jme,mesh%half_kms:mesh%half_kme))
    else if (present(half_lon) .and. present(full_lat) .and. present(full_lev)) then
      if (half_lon .and. full_lat .and. full_lev) allocate(array(member,mesh%half_ims:mesh%half_ime,mesh%full_jms:mesh%full_jme,mesh%full_kms:mesh%full_kme))
    else if (present(half_lon) .and. present(half_lat) .and. present(full_lev)) then
      if (half_lon .and. half_lat .and. full_lev) allocate(array(member,mesh%half_ims:mesh%half_ime,mesh%half_jms:mesh%half_jme,mesh%full_kms:mesh%full_kme))
    else if (present(half_lon) .and. present(full_lat) .and. present(half_lev)) then
      if (half_lon .and. full_lat .and. half_lev) allocate(array(member,mesh%half_ims:mesh%half_ime,mesh%full_jms:mesh%full_jme,mesh%half_kms:mesh%half_kme))
    else if (present(half_lon) .and. present(half_lat) .and. present(half_lev)) then
      if (half_lon .and. half_lat .and. half_lev) allocate(array(member,mesh%half_ims:mesh%half_ime,mesh%half_jms:mesh%half_jme,mesh%half_kms:mesh%half_kme))
    else
      allocate(array(member,mesh%full_ims:mesh%full_ime,mesh%full_jms:mesh%full_jme,mesh%full_kms:mesh%full_kme))
    end if
  ...
  subroutine allocate_array_3d_member_i4(mesh, array, member , full_lon, half_lon, full_lat, half_lat, full_lev, half_lev)
  ...
  end subroutine allocate_array_3d_member_i4
