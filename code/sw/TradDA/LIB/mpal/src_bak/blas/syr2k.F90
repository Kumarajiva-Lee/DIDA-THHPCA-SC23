SUBROUTINE MPAL_DSYR2K(UPLO,TRANS,N,K,ALPHA,A,LDA,B,LDB,BETA,C,LDC)
    ...
    100                 CONTINUE
                    END IF
                    DO 120 L = 1,K
                        IF ((MPAL_VAL(A(J,L)).NE.ZERO) .OR. (MPAL_VAL(B(J,L)).NE.ZERO)) THEN
                            TEMP1 = ALPHA*B(J,L)
                            TEMP2 = ALPHA*A(J,L)
                            DO 110 I = 1,J
...
SUBROUTINE MPAL_DSYR2(UPLO,N,ALPHA,X,INCX,Y,INCY,A,LDA)
        ...
        IF ((N.EQ.0) .OR. (ALPHA.EQ.ZERO)) RETURN
            ...
            IF (LSAME(UPLO,'U')) THEN
                IF ((INCX.EQ.1) .AND. (INCY.EQ.1)) THEN
                    DO 20 J = 1,N
                        IF ((MPAL_VAL(X(J)).NE.ZERO) .OR. (MPAL_VAL(Y(J)).NE.ZERO)) THEN
                            TEMP1 = ALPHA*Y(J)
                            TEMP2 = ALPHA*X(J)
                            DO 10 I = 1,J
        ...
        20         CONTINUE
                ELSE
                    DO 40 J = 1,N
                        IF ((MPAL_VAL(X(JX)).NE.ZERO) .OR. (MPAL_VAL(Y(JY)).NE.ZERO)) THEN
                            TEMP1 = ALPHA*Y(JY)
                            TEMP2 = ALPHA*X(JX)
                            IX = KX
                            IY = KY
