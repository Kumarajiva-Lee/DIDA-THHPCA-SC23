...
contains
  ...
  subroutine fiona_open_dataset(dataset_name, file_path, file_paths, parallel, mpi_comm, variant_dim)
    ...
    ierr = NF90_OPEN(dataset%file_path, NF90_NOWRITE + NF90_NETCDF4, tmp_id)
    !ierr = NF90_OPEN(dataset%file_path, NF90_NOWRITE + NF90_NETCDF4, tmp_id)
  ...
  subroutine apply_dataset_to_netcdf_master(file_path, dataset, new_file, append)
    ...
    if (merge(new_file, .true., present(new_file))) then
      ierr = NF90_CREATE(file_path, NF90_CLOBBER + NF90_NETCDF4, dataset%id)
      call handle_error(ierr, 'Failed to create NetCDF file to output!', __FILE__, __LINE__)
    else
      ierr = NF90_OPEN(dataset%last_file_path, NF90_WRITE + NF90_NETCDF4, dataset%id)
  ...
  subroutine apply_dataset_to_netcdf_slave(file_path, dataset, new_file, append)
    ...
    ierr = NF90_OPEN(file_path, NF90_WRITE + NF90_NETCDF4, dataset%id)
    call handle_error(ierr, 'Failed to open NetCDF file to output! ' // trim(NF90_STRERROR(ierr)), __FILE__, __LINE__)
  ...
  subroutine dataset_open(this)
    ...
    ierr = NF90_OPEN(trim(this%file_path), NF90_NOWRITE + NF90_NETCDF4, this%id)
