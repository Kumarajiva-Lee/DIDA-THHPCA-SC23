...
contains
  ...
  subroutine operators_init()
    ...
    case default
      call log_error('Invalid pv_scheme ' // trim(pv_scheme) // '!')
    end select
...
#ifdef Detail_Time
    call Add_Function("operators_prepare_1")
    call Indent_In()
#endif
...
#ifdef Detail_Time
    call Add_Function("operators_prepare_2")
    call Indent_In()
#endif
  ...
  subroutine calc_ph(block, dstate)
      ...
      do j = mesh%full_jds, mesh%full_jde
        do i = mesh%full_ids, mesh%full_ide
          do im = 1 , member_num
            ...
            ph_exn_lev(im,i,j,k) = ph_lev(im,i,j,k)**Rd_o_cpd
          end do  
        end do
      end do
    end do
    ...
    call fill_halo_member(block, dstate%ph_exn_lev, full_lon=.true., full_lat=.true., full_lev=.false., west_halo=.false., south_halo=.false.,async=dstate%async(async_ph_exn_lev))
    ...
    call fill_halo_member(block, dstate%ph, full_lon=.true., full_lat=.true., full_lev=.true., async=dstate%async(async_ph))
  ...
  subroutine calc_t(block, dstate)
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("calc_t")
    call Indent_In()
#endif

    call dstate%async(async_pt)%wait()   
    call dstate%async(async_ph)%wait()
    call dstate%async(async_qv)%wait()
    ...
    do k = mesh%full_kds, mesh%full_kde
      do j = mesh%full_jds, mesh%full_jde
        do i = mesh%full_ids, mesh%full_ide
          do im = 1 , member_num
            t(im,i,j,k) = temperature(pt(im,i,j,k), ph(im,i,j,k),qv(im,i,j,k))
          end do
        end do
      end do
    end do
  ...
  subroutine calc_we_lev(block, dstate, dtend, dt)
      ...
      do j = mesh%full_jds, mesh%full_jde
        do i = mesh%full_ids, mesh%full_ide
          do im = 1 , member_num
            ...
            we_lev(im,i,j,k) = - vert_coord_calc_dphdt_lev(k, dphs(im,i,j)) - mf
          end do
        end do
      end do
    end do
    ...
    call fill_halo_member(block, dstate%we_lev, full_lon=.true., full_lat=.true., full_lev=.false.,async=dstate%async(async_we_lev))
    call dstate%async(async_we_lev)%wait()
  ...
  subroutine calc_ke(block, dstate)
    ...
    real(r8) work(member_num , dstate%mesh%full_ids:dstate%mesh%full_ide,dstate%mesh%full_nlev)
    real(r8) pole(member_num , dstate%mesh%full_nlev)
    ! real(r8) order_reduce(member_num , dstate%mesh%full_ids : dtate%mesh%full_ide)


#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("calc_ke")
    call Indent_In()
#endif
  ...
  subroutine calc_div(block, dstate)
    ...
    real(r8) pole(member_num , global_mesh%full_nlev)
    integer i, j, k
    real(r8) order_reduce(member_num ,  dstate%mesh%full_ids : dstate%mesh%full_ide)
    real(r8) work(member_num , dstate%mesh%full_ids:dstate%mesh%full_ide,dstate%mesh%full_nlev)
    ...
    if (div_damp_order == 4) then
      call fill_halo_member(block, div, full_lon=.true., full_lat=.true., full_lev=.true., async=dstate%async(async_div))
    else
      call fill_halo_member(block, div, full_lon=.true., full_lat=.true., full_lev=.true., west_halo=.false., south_halo=.false., async=dstate%async(async_div))
    end if

    call dstate%async(async_div)%wait()
    ...
    call Get_Start_Time(cal_time_start)
      ...
      call fill_halo_member(block, div2, full_lon=.true., full_lat=.true., full_lev=.true., west_halo=.false., south_halo=.false., async=dstate%async(async_div2))
  ...
  subroutine calc_gz_lev(block, dstate)
    ...
    integer i, j, k, l
    !real(r8) dgz(member_num, block%mesh%full_ids:block%mesh%full_ide, block%mesh%full_jds:block%mesh%full_jde)
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("calc_gz_lev")
    call Indent_In()
#endif

    call dstate%async(async_t)%wait()
    call dstate%async(async_ph_lev)%wait()
    ...
    call fill_halo_member(block, gz_lev, full_lon=.true., full_lat=.true., full_lev=.false., async=dstate%async(async_gz_lev))
  ...
  subroutine calc_m(block, dstate)
    ...
    if (baroclinic .or. advection) then
      ...
      call fill_halo_member(block, m_lev, full_lon=.true., full_lat=.true., full_lev=.false., async=dstate%async(async_m_lev))
      ...
      call dstate%async(async_gz)%wait()
      call dstate%async(async_gzs)%wait()
    ...
    call fill_halo_member(block, m, full_lon=.true., full_lat=.true., full_lev=.true., async=dstate%async(async_m))
    call dstate%async(async_m)%wait()
    ...
    call fill_halo_member(block, m_lon, full_lon=.false., full_lat=.true., full_lev=.true., async=dstate%async(async_m_lon))
    ...
    call fill_halo_member(block, m_lat, full_lon=.true., full_lat=.false., full_lev=.true., async=dstate%async(async_m_lat))
  ...
  subroutine calc_mf(block, dstate, dt)
    ...
    call fill_halo_member(block, u_lat, full_lon=.true., full_lat=.false., full_lev=.true., async=dstate%async(async_u_lat))
    call fill_halo_member(block, v_lon, full_lon=.false., full_lat=.true., full_lev=.true., async=dstate%async(async_v_lon))
  ...
  subroutine calc_vor(block, dstate, u, v)
    ...
    real(r8) pole(member_num , dstate%mesh%full_nlev)
    integer i, j, k
    real(r8) order_reduce(member_num , dstate%mesh%half_ids : dstate%mesh%half_ide)
    real(r8) work(member_num , dstate%mesh%half_ids:dstate%mesh%half_ide,dstate%mesh%full_nlev)
  ...
  subroutine calc_pv(block, dstate)
    ...
    call fill_halo_member(block, pv, full_lon=.false., full_lat=.false., full_lev=.true., async=dstate%async(async_pv))
  ...
  subroutine interp_pv_midpoint(block, dstate, dt)
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("interp_pv_midpoint")
    call Indent_In()
#endif
    ...
    call fill_halo_member(block, pv_lat, full_lon=.true., full_lat=.false., full_lev=.true., north_halo=.false., async=dstate%async(async_pv_lat))
    ...
    call fill_halo_member(block, pv_lon, full_lon=.false., full_lat=.true., full_lev=.true., south_halo=.false., async=dstate%async(async_pv_lon))
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("interp_pv_upwind")
    call Indent_In()
#endif
      ...
      case (1)
        do k = mesh%full_kds, mesh%full_kde
          do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
            do i = mesh%half_ids, mesh%half_ide
              do im = 1 , member_num
                b  = abs(vt(im,i,j,k)) / (sqrt(un(im,i,j,k)**2 + vt(im,i,j,k)**2) + eps)
                pv_lon(im,i,j,k) = b * upwind1(sign(1.0_r8, vt(im,i,j,k)), upwind_wgt_pv, pv(im,i,j-1:j,k)) + &
          ...
          do j = mesh%half_jds, mesh%half_jde
            do i = mesh%full_ids, mesh%full_ide
              do im = 1 , member_num
                b  = abs(ut(im,i,j,k)) / (sqrt(ut(im,i,j,k)**2 + vn(im,i,j,k)**2) + eps)
                pv_lat(im,i,j,k) = b * upwind1(sign(1.0_r8, ut(im,i,j,k)), upwind_wgt_pv, pv(im,i-1:i,j,k)) + &
        ...
        do k = mesh%full_kds, mesh%full_kde
          do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
            do i = mesh%half_ids, mesh%half_ide
              do im = 1 , member_num
                b  = abs(vt(im,i,j,k)) / (sqrt(un(im,i,j,k)**2 + vt(im,i,j,k)**2) + eps)
                pv_lon(im,i,j,k) = b * upwind3(sign(1.0_r8, vt(im,i,j,k)), upwind_wgt_pv, pv(im,i,j-2:j+1,k)) + &
                                   (1 - b) * 0.5_r8 * (pv(im,i,j-1,k) + pv(im,i,j,k))
              end do  
            end do
          end do
          do j = mesh%half_jds, mesh%half_jde
            do i = mesh%full_ids, mesh%full_ide
              do im = 1 , member_num
                b  = abs(ut(im,i,j,k)) / (sqrt(ut(im,i,j,k)**2 + vn(im,i,j,k)**2) + eps)
                pv_lat(im,i,j,k) = b * upwind3(sign(1.0_r8, ut(im,i,j,k)), upwind_wgt_pv, pv(im,i-2:i+1,j,k)) + &
                                   (1 - b) * 0.5_r8 * (pv(im,i-1,j,k) + pv(im,i,j,k))
              end do
            end do
          end do
  ...
  subroutine interp_pv_tvd(block, dstate, dt)
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("interp_pv_tvd")
    call Indent_In()
#endif
    ...
    do k = mesh%full_kds, mesh%full_kde
      do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
        do i = mesh%half_ids, mesh%half_ide
          do im = 1 , member_num
            cfl = vt(im,i,j,k) * dt / mesh%le_lon(j)
            pv_lon(im,i,j,k) = tvd(cfl, pv(im,i,j-2,k), pv(im,i,j-1,k), pv(im,i,j,k), pv(im,i,j+1,k))
          end do
        end do
      end do
      do j = mesh%half_jds, mesh%half_jde
        do i = mesh%full_ids, mesh%full_ide
          do im = 1 , member_num
            cfl = ut(im,i,j,k) * dt / mesh%le_lat(j)
            pv_lat(im,i,j,k) = tvd(cfl, pv(im,i-2,j,k), pv(im,i-1,j,k), pv(im,i,j,k), pv(im,i+1,j,k))
          end do
        end do
      end do
    end do
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("calc_coriolis")
    call Indent_In()
#endif
...
#ifdef Detail_Time
    call Get_Start_Time(cal_time_start)
    call Add_Function("calc_grad_ke")
    call Indent_In()
#endif
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("calc_grad_mf")
    call Indent_In()
#endif
  ...
  subroutine calc_grad_ptf(block, dstate, dtend, dt)
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("calc_grad_ptf")
    call Indent_In()
#endif
    call dstate%async(async_pt)%wait()
    ...
    call fill_halo_member(block, ptf_lon, full_lon=.false., full_lat=.true., full_lev=.true., &
                          south_halo=.false., north_halo=.false., east_halo=.false., &
                          async=dstate%async(async_ptf_lon))
    call fill_halo_member(block, ptf_lat, full_lon=.true., full_lat=.false., full_lev=.true., &
                          north_halo=.false.,  west_halo=.false., east_halo=.false., &
                          async=dstate%async(async_ptf_lat))
    call dstate%async(async_ptf_lon)%wait()
    call dstate%async(async_ptf_lat)%wait()
...
#ifdef Detail_Time
    call Get_Start_Time(tran_time_start)
    call Add_Function("calc_wedudlev_wedvdlev")
    call Indent_In()
#endif
