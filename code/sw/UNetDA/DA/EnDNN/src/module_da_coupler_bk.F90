...
subroutine dafatm_coupling(MPI_DA_GROUP)
  ...
  ens_id = 1
  write(s_id,"(i3.3)") ens_id
  call fiona_open_dataset('i0', file_paths=init_data_paths, mpi_comm=MPI_DA_GROUP)
  do i = 1, cnn_ensemble
    ...
    ks = 1; ke = num_lev
    nc_start = [is,js,ks]
    nc_count = [nx,ny,num_lev]

    call fiona_input('i0', 'phs', da_data_t(i,1:nx,1:ny,1), start=nc_start, count=nc_count)
    !call log_notice("input phs")
    call fiona_input('i0', 'pt' , da_data_t(i,1:nx,1:ny,65+ks:65+ke), start=nc_start, count=nc_count)
    ...
    call fiona_input('i0', 'u_Agrid', da_data_t(i,1:nx,1:ny,1+ks:1+ke), start=nc_start, count=nc_count)
    !call log_notice("input_u")
    call fiona_input('i0', 'v_Agrid', da_data_t(i,1:nx,1:ny,33+ks:33+ke), start=nc_start, count=nc_count)
    !call log_notice("input_v")
    call fiona_end_input('i0')
    
    call log_notice("nc_input member "//to_str(i))
